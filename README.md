...
- **Clean Showfile** -- Walks a curated list of content pools (Macros, Presets, Groups, Sequences, Effects, Worlds, Filters, Layouts, Views, Timecodes, Pages) and asks a Yes/No dialog for each one, e.g. *"Delete all macros?"*. Answers are collected first; after a final summary confirmation the selected pools are wiped in one batch. Closing any dialog (the X) aborts the whole run without deleting anything. Patch, fixtures, and DMX are deliberately excluded.
