...
- **Clean Showfile** -- Walks a curated list of content pools (Macros, Presets, Groups, Sequences, Effects, Worlds, Filters, Layouts, Views, Timecodes, Pages), counting each pool live, and asks for each one, e.g. *"Delete all 14 macros?  (yes / no / cancel)"* (prefilled `no`, so Enter skips). `yes` marks the pool, `no` skips it, `cancel` aborts the whole run. Answers are collected first; after a final summary confirmation the selected pools are wiped in one batch. Patch, fixtures, and DMX are deliberately excluded.
